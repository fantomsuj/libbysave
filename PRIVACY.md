...
- Optional library directory metadata (domain, location, and consortium status) for selected libraries
- Up to 30 recent library-search result sets, cached locally for seven days
...
Library finder queries are sent directly to Libby's official locator service at `locate.libbyapp.com`. If the input looks like a slug or OverDrive domain, LibbySave may also send that identifier to OverDrive's catalog service to resolve it. The extension does not request device geolocation, infer a location, or send selected-library settings to the developer.
...
The extension asks only for site access needed for its core behavior: Open Library search, the supported book-list pages (including only `https://open.spotify.com/*` for Spotify), Libby and its locator subdomain, and the OverDrive catalog endpoint. It does not request geolocation or any new sensitive Chrome permission.
